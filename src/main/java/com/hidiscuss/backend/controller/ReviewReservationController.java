...
public class ReviewReservationController {
    ...
    public List<ReviewReservation> isReserved(@RequestParam("discussionId") Long discussionId) {
        if (discussionService.findByIdOrNull(discussionId) == null) {
            throw NotFoundDiscussion();
        }

        Long userId = 1L;
        List<ReviewReservation> result = reviewReservationService.findByDiscussionIdAndUserId(discussionId, userId);
        return result;
    }
